...
HitsukayaCart.dataTable = { routes: {}, selected: {} };
...
export default class {
    constructor(selector, options, callback) {
        ...
        if (HitsukayaCart.dataTable.selected[selector] === undefined) {
            HitsukayaCart.dataTable.selected[selector] = [];
        }
    ...
    appendToSelected(id) {
        ...
        if (! HitsukayaCart.dataTable.selected[this.selector].includes(id)) {
            HitsukayaCart.dataTable.selected[this.selector].push(id);
        }
    }
    ...
    removeFromSelected(id) {
        ...
        HitsukayaCart.dataTable.selected[this.selector].remove(id);
    }
    ...
    route(name, params) {
        let router = HitsukayaCart.dataTable.routes[this.selector][name];
    ...
    hasRoute(name) {
        return HitsukayaCart.dataTable.routes[this.selector][name] !== undefined;
    }

    static setRoutes(selector, routes) {
        HitsukayaCart.dataTable.routes[selector] = routes;
    }

    static setSelectedIds(selector, selected) {
        HitsukayaCart.dataTable.selected[selector] = selected;
    }

    static getSelectedIds(selector) {
        return HitsukayaCart.dataTable.selected[selector];
    }
