...
use Modules\Core\Foundation\Asset\Manager\HitsukayaCartAssetManager;
use Modules\Core\Foundation\Asset\Pipeline\HitsukayaCartAssetPipeline;
...
class AssetServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(AssetManager::class, HitsukayaCartAssetManager::class);

        $this->app->singleton(AssetPipeline::class, function ($app) {
            return new HitsukayaCartAssetPipeline($app[AssetManager::class]);
        });
    }
    ...
    private function addThemesAssets()
    {
        ...
        if (! is_null($assets = config("HitsukayaCart.themes.{$theme}.assets"))) {
    ...
    private function addModulesAssets()
    {
        foreach ($this->app['modules']->allEnabled() as $module) {
            $assets = config("HitsukayaCart.modules.{$module->getAlias()}.assets");
